...
try {
  ...
  const videoPanel = boards.includes('assets/competitive-dynamics.mp4?v=')
    ...
    && boards.includes('className="dyn-layer-head"')
    && boards.includes("relationshipCoverageCount")
    && boards.includes('aria-label="AI 밸류체인별 업체"')
    && boards.includes('activeLayer === layer.id')
    && boards.includes('const scopeCompanies = activeLayer === "all"')
    && boards.includes('pickerCompanies: scopeCompanies')
    && boards.includes('scopedEdges: layerEdges')
    ...
    && boards.includes("const isInActiveLayer = pickerCompanies.some")
    && boards.includes("hasVisibleRelationship")
    && boards.includes('className="dyn-filter-context"')
    && boards.includes(".filter(axis => axis.items.length > 0)")
  ...
  const interactiveLayout = styles.includes(".es-dynamics-grid")
    ...
    && styles.includes(".dyn-layer-head")
    && styles.includes(".dyn-layer-index")
    && styles.includes(".dyn-layer-copy")
    && styles.includes(".dyn-company-picker")
    && styles.includes(".dyn-axis-list button")
    && styles.includes(".dyn-filter-context")
    && !styles.includes(".dyn-axis-empty")
...
try {
  ...
  const dataDrivenInvestmentCatalog = boards.includes('const chains = data?.valueChains || []')
    && boards.includes('className="nvi-chain-nav"')
    && boards.includes('className="nvi-company-grid"')
    && boards.includes("formatMonthDay")
