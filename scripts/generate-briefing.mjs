...
function briefPrompt(cands) {
  ...
  return `아래는 오늘 수집된 AI 산업 기사 후보입니다. 이 중 서로 다른 주제 ${MAX_ITEMS}개를 골라 '모닝 브리핑' 카드를 작성하세요.

${list}

각 카드(JSON):
- signal: 시장에서 관측된 사실(1문장, 수치 포함 우선)
- insight: 왜 중요한가 — 단말(스마트폰·노트북·웨어러블) 사업 관점의 해석(1~2문장)
- action: 다음 액션 — 검토·추적·협상 등 실행 지향 제안(1문장)
...
headline: 오늘 브리핑 전체를 관통하는 한 줄(25자 내외).
...
function ruleBriefing(cands) {
  ...
  return { engine: "rules", headline: "오늘의 AI 산업 핵심 신호", items };
}
...
async function main() {
  ...
  const today = { date: TODAY, headline: brief.headline, engine: brief.engine, items: brief.items };
