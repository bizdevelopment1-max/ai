...
function MobileAIBusinessBoard({ sectionRef, dataVersion }) {
  ...
  const SignalCard = ({ signal }) => (
    <button className={`mxc-signal-card${selected?.id === signal.id ? " is-selected" : ""}`} aria-pressed={selected?.id === signal.id} onClick={() => setSelectedId(signal.id)} data-testid={`mx-signal-${signal.id}`}>
