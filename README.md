...
The production build minifies JavaScript with dead-code removal and emits a separate `styles.bundle.css` while retaining readable JSX and `styles.css` as authoring sources. The initial route downloads only `overview-view.json`; news, research, company, startup, market, and media payloads are requested after the corresponding section becomes active. `npm run validate:delivery` blocks publication when the app, CSS, taxonomy, or overview payload exceeds its delivery budget, when the compact overview carries duplicated article summaries, or when production output contains legacy fixed market facts.
